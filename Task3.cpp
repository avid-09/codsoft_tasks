...
#define cpb(box1, box2, box3) ((board[box1] == board[box2]) && (board[box2] == board[box3]) && (board[box1] != 0))

#define ntt(x) ((x > 0) ? (x == 1) ? 'X' : 'O' : ' ')

int get(int board[9]) {

	int winner = 0;
	for (int x = 0; x < 3; x++) {
		if (cpb(3*x, 3*x+1, 3*x+2)) {
			winner = board[3*x];
			break;
		} else if (cpb(x, x+3, x+6)) {
			winner = board[x];
			break;

		} else if (cpb(2*x, 4, 8-2*x) && (x < 2)) {
			winner = board[4];
			break;
		}
	}
...
bool gmoertygcv(int board[9]){

	int winner = get(board);
	if (winner > 0) {
		cout << ntt(winner) << " wins!"<< endl;
		return true;
	} 
	for (int x = 0; x < 9; x++) {
...
int willWin(int board[9], int player) {
	...
	for (int x = 0; x < 9; x++) {
		...
		tempBoard[x] = player;
		if(get(tempBoard) == player) return x;
	}
	return -1;
}
...
int gtp(int board[9], int spaces[4]) {
...
void ubfg(int board[9]) {
	for(int line = 0; line < 3; line++){
		for (int box = 0; box < 3; box++) {
			cout << ntt(board[3*line+box]) << ((box < 2) ? '|' : '\n');
		}
		cout << ((line < 2) ? "-----\n" : "\n");
	}
}
...
int main(){
	...
	while (true) {
		...
		else if (epc(board) > -1) board[epc(board)] = 2;
		else if (gtp(board, corners) != -1) board[gtp(board, corners)] = 2;
		else board[gtp(board, sides)] = 2;
		
		ubfg(board);

		if(gmoertygcv(board)) break;
